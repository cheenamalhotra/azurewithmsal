...
export function getAzureActiveDirectoryConfig(): AzureAuthType | undefined {
	...
	if (config) {
		...
		if (val) {
			return val === AzureAuthType[AzureAuthType.AuthCodeGrant]
				? AzureAuthType.AuthCodeGrant : AzureAuthType.DeviceCode;
		}
	} else {
...
export function getAzureAuthLibraryConfig(): AuthLibrary {
	...
	if (config) {
		...
		if (val) {
			return val === AuthLibrary[AuthLibrary.ADAL]
				? AuthLibrary.ADAL : AuthLibrary.MSAL;
		}
	}
