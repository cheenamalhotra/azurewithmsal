...
import { AuthenticationTypes, IAzureSignInQuickPickItem, IConnectionProfile } from './interfaces';
...
export function getConfigPiiLogging(): boolean | undefined {
	let config = getConfiguration();
	if (config) {
...
export function getCommonLaunchArgsAndCleanupOldLogFiles(logPath: string, fileName: string, executablePath: string): (string | undefined)[] {
	let launchArgs = [];
