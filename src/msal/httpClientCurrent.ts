		...
		request.on('connect', (response, socket) => {
			...
			socket.on('end', () => {
				...
				if (((httpStatusCode < HttpStatus.SUCCESS_RANGE_START) || (httpStatusCode > HttpStatus.SUCCESS_RANGE_END)) &&
					...
					(networkResponse.body as ErrorResponse)['error'] !== constants.AUTHORIZATION_PENDING) {
					request.destroy();
				}
 ...
 */
const parseBody = (statusCode: number, statusMessage: string | undefined, headers: Record<string, string>, body: string): ErrorResponse | BodyResponse => {
	/*
	...
	let parsedBody: ErrorResponse | BodyResponse;
	try {
